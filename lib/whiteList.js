...
function WhiteList() {
	...
	console.log(this.customWhiteListFile);
...
WhiteList.prototype.saveOfficial = function(versionData, whitelistData) {
	try {
		fs.writeFileSync(this.officialWhiteListFile, JSON.stringify(whitelistData));
		fs.writeFileSync(this.officialWhiteListVersion, JSON.stringify(versionData));
		return null;
	} catch (e) {
		return e;
	}
